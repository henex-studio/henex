...
**Next action:** All F1-F21 items are coded and statically validated (13.8 self-audit passed, brace/tag/duplicate-id checks clean across all 5 touched files). ASH and STD are ready for Cafe's light+dark re-walkthrough. Only after that gate does M-2 (NDS+RKT) start, applying the F-series lessons — the shared classes (`.ux-nav-card`, `.ux-glyph-card`, `.ux-desc-aligned`, spacing utilities), the F6 dark-token derivation formulas, and the Section 13.8 checklist — from the outset, so this defect class should not recur.
